    ...
    for counter in range(n_batches):
        ...
        for _ in range(n_inner_iter):
            ...
            if cfg.OPTIM.ADAPT == "ent":
                ...
                print("Mentioned Adaptation Loss Function Not Implemented. Please check for correct arguments")
                pass
...
test_data = datasets.MNIST('./data', train=False, download=True,
                transform=transform)
train_loader = torch.utils.data.DataLoader(test_data,batch_size=10000,shuffle=False)
for batch_idx, (data,labels) in enumerate(train_loader):
    x_test, y_test = data.cuda(), labels.cuda()
    break
x_test = x_test.expand(x_test.shape[0], 3, 28, 28)
...
err = 1. - acc
logger.info(f"Error on Target Domain {err:.2%}")